	...
	throws SAXException
	{
       ...
       throws SAXException
       {
            if(activeTag.equals("Message")) {
                  if(values.hearMessage[hearFrom]==null)
                      values.hearMessage[hearFrom]=new String(ch,start,length);
                  else 
                      values.hearMessage[hearFrom]+=new String(ch,start,length);
            }
       }
...
class ReplyHandler extends DefaultHandler {
	...
	throws SAXException
	{
	    ...
	    if(qName.equals("Parameters")) {     // Parameters Values
		      ...
		      String runningTimeoutStr=attrs.getValue("RunningTimeout");   // RunningTimeout
	              if(runningTimeoutStr!=null) {
		          params.runningTimeout = Integer.valueOf(runningTimeoutStr).intValue();
		      }
...
public class ciberIF {
    ...
    */
    ...
    public boolean NewMessageFrom(int id) {
            if( values.hearMessage[id]==null) return false;
	    return !values.hearMessage[id].equals("");
    }
